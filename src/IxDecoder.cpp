...
cudaVideoCodec IxDecoder::GetCodec() const { return p_impl->m_eCodec; }
 ...
 */
int IxDecoder::HandleVideoSequence(CUVIDEOFORMAT* pVideoFormat) noexcept
{
  try {
...
int IxDecoder::ReconfigureDecoder(CUVIDEOFORMAT* pVideoFormat)
{
  CudaCtxPush ctxPush(p_impl->m_cuContext);
 ...
 *   >=1: suceeded
 */
int IxDecoder::HandlePictureDecode(CUVIDPICPARAMS* pPicParams) noexcept
{
  try {
 ...
 *   >=1: suceeded
 */
int IxDecoder::HandlePictureDisplay(CUVIDPARSERDISPINFO* pDispInfo) noexcept
{
  try {
    CudaCtxPush ctxPush(p_impl->m_cuContext);
...
IxDecoder::IxDecoder(CUstream cuStream, CUcontext cuContext,
...
int IxDecoder::GetWidth() { return p_impl->m_nWidth; }

int IxDecoder::GetHeight() { return p_impl->m_nLumaHeight; }

int IxDecoder::GetChromaHeight()
{
  return p_impl->m_nChromaHeight * p_impl->m_nNumChromaPlanes;
}

int IxDecoder::GetFrameSize()
{
...
int IxDecoder::GetDeviceFramePitch()
{
  return p_impl->m_nDeviceFramePitch ? (int)p_impl->m_nDeviceFramePitch
                                     : p_impl->m_nWidth * p_impl->m_nBPP;
}

int IxDecoder::GetBitDepth() { return p_impl->m_nBitDepthMinus8 + 8; }

cudaVideoChromaFormat IxDecoder::GetChromaFormat() const
{
  return p_impl->m_eChromaFormat;
}

bool IxDecoder::DecodeLockSurface(Buffer const* encFrame,
...
void IxDecoder::UnlockSurface(CUdeviceptr& lockedSurface)
{
  if (lockedSurface) {
    lock_guard<mutex> lock(p_impl->m_mtxVPFrame);
