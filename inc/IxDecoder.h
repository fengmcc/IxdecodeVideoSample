...
public:
  IxDecoder() = delete;
  IxDecoder(const IxDecoder& other) = delete;
  IxDecoder& operator=(const IxDecoder& other) = delete;

  IxDecoder(CUstream cuStream, CUcontext cuContext, cudaVideoCodec eCodec,
...
private:
  ...
  {
    return ((IxDecoder*)pUserData)->HandleVideoSequence(pVideoFormat);
  }

  static int  
  HandlePictureDecodeProc(void* pUserData, CUVIDPICPARAMS* pPicParams) noexcept
  {
    return ((IxDecoder*)pUserData)->HandlePictureDecode(pPicParams);
  }

  static int   HandlePictureDisplayProc(
      void* pUserData, CUVIDPARSERDISPINFO* pDispInfo) noexcept
  {
    return ((IxDecoder*)pUserData)->HandlePictureDisplay(pDispInfo);
  }
